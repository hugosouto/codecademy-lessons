...
adelie = pd.read_csv('data-science-foundations/data/adelie.csv')
adelie['Species'] = 'Adelie'
gentoo = pd.read_csv('data-science-foundations/data/gentoo.csv')
gentoo['Species'] = 'Gentoo'
chinstrap = pd.read_csv('data-science-foundations/data/chinstrap.csv')
...
km_model = KMeans(n_clusters = 3, random_state=17, n_init='auto').fit(penguins[['Flipper Length','Bill Length']].dropna().values)
...
# Define a mystery penguin's flipper and bill lengths
